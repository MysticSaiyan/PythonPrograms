...
def listMaker(instructions): # made list of sequence
	offsetList = list(map(int, instructions.split()))
	return offsetList

offsetList = listMaker(instructions)

def DictMaker(testlist): # made dict of sequence for better understanding
	l = len(testlist)
	dict = {}
	for i in range(l):
		dict[i] = testlist[i]
...
offsetDict = DictMaker(offsetList)


offsetDict1 = offsetDict.copy()
offsetDict2 = offsetDict.copy()
...
def stepsCalc(offsetDict):
	"""if you are on number n, jump ahead n numbers and increment n by 1
		calculate steps needed to go out of the number list
	"""
	steps = 0
	pos = 0
	try:
		while True:
			offsetDict[pos] = offsetDict[pos] + 1
			pos += (offsetDict.get(pos) - 1)
			steps += 1
	except KeyError:
		return steps
...
def stepsCalc2(offsetDict):
	"""if you are on number n, jump ahead n numbers and 
		if n < 3, increment n by 1
		if n >= 3, decrement n by 1
		calculate steps needed to go out of the number list
	"""
	steps = 0
	pos = 0
