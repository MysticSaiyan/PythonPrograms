...
passLines = passphrases.splitlines()

def passValidator(passLines):
	"""passLines is a list of various test passphrase sentences
	First we split the words of a line and then test if it occurs again in the same line.
	If it doesn't it is a valid passphrase and added to total count which is returned.
	"""
	validPhrases = 0
	for phrase in passLines:
		passWords = phrase.split(" ")
		test = True
		for i in range(len(passWords)):
			for j in range(len(passWords)):
				if passWords[i] == passWords[j] and i != j:
					test = False
		if test:
			validPhrases += 1
	return validPhrases

print(passValidator(passLines))
...
def passValidator2(passLines):
	"""passLines is a list of various test passphrase sentences
	First we split the words of a line and 
	then test if its letters occur in another word in same line.
	If they don't its a valid passPhrase and added to total.
	"""
	validPhrases = 0
	for phrase in passLines:
		passWords = phrase.split(" ")
		...
		for i in range(len(passWords)):
			for j in range(len(passWords)):
				if checkAnagram(passWords[i],passWords[j]) and i != j:
					test = False
					break
		if test:
			validPhrases += 1
...
print(passValidator2(passLines))
